...
export default function Studies() {
  ...
  return (
    <section id="academic" className="py-16 md:py-32 px-4 md:px-20 bg-surface/30 backdrop-blur-3xl border-t border-white/5 overflow-hidden relative">
      <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-6 md:gap-12">
        ...
        <div className="lg:w-1/3 relative lg:sticky lg:top-32 h-fit text-left">
          <span className="font-sans text-[10px] md:text-xs text-accent uppercase tracking-[0.4em] font-black border border-accent/20 px-3 py-2 rounded-full mb-4 md:mb-8 inline-block">Career Path</span>
          <h2 className="text-4xl sm:text-5xl md:text-7xl font-display font-medium uppercase tracking-tighter text-white leading-tight md:leading-none">Education <br className="hidden md:block" />& <br className="hidden md:block" /><span className="text-accent underline decoration-white/10">Experience</span></h2>
        </div>
        ...
        <div className="lg:w-2/3 flex flex-col gap-12 md:gap-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-8 text-left">
            <div className="col-span-1 sm:col-span-2">
              <h3 className="font-sans text-[10px] uppercase tracking-[0.5em] text-white/40 font-bold mb-4">Academic Journey</h3>
            </div>
            {education.map((edu, idx) => (
              <motion.div 
                ...
                viewport={{ once: true }}
                className="relative p-4 sm:p-5 md:p-10 border border-white/10 bg-background/50 rounded-[1.75rem] group overflow-hidden"
              >
                <div className="absolute top-6 right-6 opacity-5 group-hover:opacity-15 transition-opacity text-accent">
                  {edu.icon}
                </div>

                <div className="flex flex-col gap-4 relative z-10 text-left">
                  <div className="flex flex-wrap items-center gap-3 text-[9px] font-sans text-white/40 tracking-[0.3em] uppercase font-bold">
                    {edu.period && <span>{edu.period}</span>}
                    {edu.period && <span className="hidden sm:block w-10 h-[1px] bg-accent/30"></span>}
                    <span className="text-accent">{edu.status}</span>
                  </div>
                  <div className="flex flex-col gap-2">
                    <h3 className="text-xl sm:text-2xl md:text-4xl font-display font-medium uppercase text-white leading-tight">
                      {edu.degree}
                    </h3>
                    <span className="text-sm md:text-lg text-gray-500 font-sans tracking-wide">{edu.school}</span>
                  </div>
                  <p className="hidden sm:block text-gray-500 font-sans italic opacity-80 leading-relaxed text-xs md:text-sm">
                    "{edu.description}"
                  </p>
                </div>
              </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-8 text-left">
            <div className="col-span-1 sm:col-span-2">
              <h3 className="font-sans text-[10px] uppercase tracking-[0.5em] text-white/40 font-bold mb-4">Industry Experience</h3>
            </div>
            {internships.map((intern, idx) => (
              <motion.div 
                ...
                transition={{ delay: idx * 0.2, duration: 1, ease: [0.76, 0, 0.24, 1] }}
                viewport={{ once: true }}
                className="relative p-4 sm:p-5 md:p-10 border border-accent/10 bg-accent/5 rounded-[1.75rem] group overflow-hidden"
              >
                <div className="flex flex-col gap-4 relative z-10 text-left">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-accent font-sans text-[9px] uppercase tracking-widest font-black">Internship</span>
                    <span className="text-white/20 text-[9px]">•</span>
                    <span className="text-white/40 font-sans text-[9px] uppercase tracking-widest flex items-center gap-2">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <span className="text-sm md:text-lg text-gray-400 font-sans tracking-wide">{intern.company}</span>
                  </div>
                  <ul className="flex flex-col gap-2 mt-2">
                    {intern.points.map((point, i) => (
                      <li key={i} className={`flex items-start gap-3 text-gray-400 font-sans ${i > 1 ? 'hidden sm:flex' : 'flex'} text-[11px] md:text-sm leading-relaxed`}>
                  ...
                  {intern.certFile && (
                    <div className="mt-4">
                      <button
                        ...
                        className="inline-flex items-center gap-2 font-sans font-bold text-[10px] uppercase tracking-widest bg-accent/10 border border-accent/20 text-accent hover:bg-accent hover:text-black transition-all duration-300 px-4 py-2 rounded-full"
                      >
